...
export class ApiService {
   ...
   */
  static async get<T>(url: string, getHeaders: () => Promise<Record<string, string>>, config?: AxiosRequestConfig): Promise<T> {
    try {
   ...
   * @returns The response data
   */
  static async post<T>(url: string, data: any, getHeaders: () => Promise<Record<string, string>>, config?: AxiosRequestConfig): Promise<T> {
    try {
      const headers = await getHeaders();
