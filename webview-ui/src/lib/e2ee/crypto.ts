// import * as vscode from 'vscode'; // Cannot use vscode here
...
export async function initializeSodium(): Promise<typeof sodium> {
    ...
    try {
        await sodium.ready;
        _sodium = sodium;
        console.log('libsodium-wrappers initialized successfully (WebUI).');
        return _sodium;
    } catch (error) {
        console.error('Failed to initialize libsodium-wrappers (WebUI):', error);
        // Cannot use vscode.window.showErrorMessage here
        alert('Fatal Error: Cryptographic library failed to load. E2EE cannot function.'); // Use alert as fallback
...
export function hashData(
    ...
    // Use the known default value directly instead of referencing sodium instance here
    outputLength: number = 32 // sodium.crypto_generichash_BYTES is typically 32
): Uint8Array {
    const sodium = getSodium();
    // Verify the constant value if needed (optional)
    // if (outputLength === 32 && sodium.crypto_generichash_BYTES !== 32) {
    //     console.warn("Sodium generichash default bytes mismatch!");
    // }
